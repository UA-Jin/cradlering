...
impl VectorStore {
    ...
    pub fn list(&self, limit: Option<usize>) -> Vec<VectorRecord> {
        let inner = self.inner.read().unwrap();
        let mut recs: Vec<VectorRecord> = inner.records.clone();
        recs.sort_by(|a, b| {
            let ta = a.metadata.get("createdAt").and_then(|v| v.as_i64()).unwrap_or(0);
            let tb = b.metadata.get("createdAt").and_then(|v| v.as_i64()).unwrap_or(0);
            tb.cmp(&ta)
        });
        if let Some(limit) = limit { recs.truncate(limit); }
        recs
    }
    ...
    pub fn search(
        ...
        let inner = self.inner.read().unwrap();
        let mut scored: Vec<VectorSearchHit> = inner.records.iter()
    ...
    pub fn keyword_search(&self, query: &str, top_k: usize, filter: &MetadataFilter) -> Vec<VectorSearchHit> {
        ...
        let mut scored: Vec<VectorSearchHit> = inner.records.iter()
            ...
            .map(|r| {
                let overlap = lcs_overlap(&q, &r.text.to_lowercase());
                VectorSearchHit { score: overlap, record: r.clone() }
            })
            .collect();
        scored.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
        scored.truncate(top_k);
    ...
    pub fn increment_hit(&self, id: &str) -> anyhow::Result<()> {
        ...
        if let Some(&pos) = inner.index.get(id) {
            let hits = inner.records[pos].metadata.entry("hitCount".to_string())
